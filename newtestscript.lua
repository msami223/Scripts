...
-- Construct full URLs using the base URL and distributor ID
...
-- Create the key verification UI - Based on your original structure
local function createKeyUI()
    ...
    -- We also need a label for the Distributor ID, even if it wasn't in your exact sample image
    ...
    MainFrame.Size = UDim2.new(0, 300, 0, 200) -- Keeping the original size
    ...
    -- Configure Distributor Label (Placed just below Title in this UI structure)
    DistributorLabel.Name = "DistributorLabel"
    DistributorLabel.Parent = MainFrame
    DistributorLabel.BackgroundTransparency = 1
    DistributorLabel.Position = UDim2.new(0, 0, 0, 30) -- Position immediately below the title
    DistributorLabel.Size = UDim2.new(1, 0, 0, 20) -- Given some height
    DistributorLabel.Font = Enum.Font.Gotham
    DistributorLabel.Text = "Distributor ID: " .. DISTRIBUTOR_ID -- Display distributor ID
    DistributorLabel.TextColor3 = Color3.fromRGB(180, 180, 255) -- A subtle color
    DistributorLabel.TextSize = 10.000 -- Smaller text size
    DistributorLabel.TextXAlignment = Enum.TextXAlignment.Center -- Center the ID
    DistributorLabel.TextWrapped = true


    -- Configure KeyInput - Position adjusted down to make space for DistributorLabel
    KeyInput.Name = "KeyInput"
    KeyInput.Parent = MainFrame
    KeyInput.BackgroundColor3 = Color3.fromRGB(51, 65, 85)
    KeyInput.BorderSizePixel = 0
    KeyInput.Position = UDim2.new(0.5, -125, 0, 55) -- Adjusted Y position (below Title + Label)
    ...
    -- Configure SubmitButton - Position adjusted down
    SubmitButton.Name = "SubmitButton"
    SubmitButton.Parent = MainFrame
    SubmitButton.BackgroundColor3 = Color3.fromRGB(34, 197, 94) -- Green
    SubmitButton.BorderSizePixel = 0
    SubmitButton.Position = UDim2.new(0.5, -60, 0, 95) -- Adjusted Y position (below KeyInput)
    ...
    -- Configure GetKeyButton - Position adjusted down
    GetKeyButton.Name = "GetKeyButton"
    GetKeyButton.Parent = MainFrame
    GetKeyButton.BackgroundColor3 = Color3.fromRGB(51, 65, 85) -- Blueish gray
    GetKeyButton.BorderSizePixel = 0
    GetKeyButton.Position = UDim2.new(0.5, -60, 0, 135) -- Adjusted Y position (below SubmitButton)
    ...
    -- Configure StatusLabel - Position adjusted down
    StatusLabel.Name = "StatusLabel"
    StatusLabel.Parent = MainFrame
    StatusLabel.BackgroundTransparency = 1
    StatusLabel.Position = UDim2.new(0, 0, 0, 175) -- Adjusted Y position (below GetKeyButton)
...
local function verifyKey(key, distId) -- Added distId parameter
    -- Construct the verification URL using the base URL and parameters
    local fullVerificationUrl = verificationUrl .. "&key=" .. key .. "&dist_id=" .. distId -- Use dist_id parameter name
    ...
    if success then
        ...
        -- Server side is NOT currently checking or returning 'used' during verification in the plugin
...
local function initKeySystem()
    ...
    ui.GetKeyButton.MouseButton1Click:Connect(function()
        -- The getKeyUrl already includes the distributorId and uses the correct parameter name
    ...
    ui.SubmitButton.MouseButton1Click:Connect(function()
        ...
        task.delay(1.0, function() -- Reduced delay slightly
            local isValid, status = verifyKey(key, DISTRIBUTOR_ID) -- Pass key and distributorId to verification
